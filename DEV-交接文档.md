...
> 文档状态：**历史交接快照（2026-07-02）**。其中版本、数量、待办、分支和发布描述可能已过期，不得覆盖 [`AGENTS.md`](AGENTS.md) 与 [`docs/index.md`](docs/index.md) 的当前规则。
>
> 给协作开发 agent(Codex，可读写本地文件、操作 Supabase / VSCode / GitHub / Vercel)的技术上手文档。
> 你和另一位开发 agent（在 Cowork 里）职能相同、共同协作：额度紧张时互相接力。
> **当前纪律以 `AGENTS.md` 为准：本地修改、提交、push和部署分开授权，每次push必须明确批准。**
...
- 获得明确授权后 push 并由 Vercel 完成部署 → **远程改动才会对用户生效**。
...
3. **小步提交**（一个逻辑改动一个 commit）。push、合并和部署必须单独获得明确授权；中高风险任务原则上走独立分支或审核流程。
