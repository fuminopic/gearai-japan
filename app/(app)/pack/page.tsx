...
import { PackRemoveButton } from "@/components/pack-remove-button";
...
export default async function PackPage() {
  ...
  const groups = groupPackGearByCategory(pack.items);

  return (
    <div className="space-y-5">
      <section className="flex items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-forest-700">いつもの装備</p>
      ...
      <section className="grid grid-cols-3 overflow-hidden rounded-2xl bg-white shadow-sm">
        <PackStat label="包内装備" value={`${pack.summary.itemCount.toLocaleString("ja-JP")}点`} />
        <PackStat
          label="既知の総重量"
          value={pack.summary.knownWeightG > 0 ? formatWeight(pack.summary.knownWeightG) : "-"}
          divided
        />
        <PackStat
          label="重量未入力"
          value={`${pack.summary.missingWeightCount.toLocaleString("ja-JP")}点`}
          divided
        />
      </section>
            ...
            <section key={group.id}>
              ...
              <div className="mt-2 space-y-2">
                {group.items.map((item) => (
                  <PackGearRow key={item.id} item={item} />
                ))}
              </div>
            </section>
          ))}
...
function PackGearRow({ item }: { item: UserGear }) {
  ...
  return (
    <article className="flex items-center gap-3 rounded-2xl bg-white p-3 shadow-sm">
      ...
      <PackRemoveButton gearId={item.id} />
    </article>
  );
}
